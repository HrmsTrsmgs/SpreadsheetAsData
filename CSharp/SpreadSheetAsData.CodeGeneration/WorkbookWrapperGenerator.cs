...
public static class WorkbookWrapperGenerator
{
    ...
    static IEnumerable<CodeGenerationDiagnostic> BookDefinedNameDiagnostics(
        ...
        where propertyName == $"{Path.GetFileNameWithoutExtension(filePath).ToCSharpIdentifier()}Book"
            || propertyName is nameof(Workbook.Read) or nameof(Workbook.Open)
                ...
                or nameof(Workbook.DefinedNames) or nameof(Workbook.Sheets)
                or nameof(Workbook.Save) or nameof(Workbook.SaveAs)
                or nameof(Workbook.Close) or nameof(Workbook.Dispose) or nameof(Workbook.ReadTable)
        select new CodeGenerationDiagnostic(
            true,
            propertyName,
            [definedName.Name]);
    ...
    static IEnumerable<CodeGenerationDiagnostic> BookDefinedNamePropertyDiagnostics(
        ...
        from sourceName in
            Enumerable.Concat(
                from sheet in book.Sheets.Values select sheet.Name,
                from table in book.Tables select table.Name)
        where propertyName == options.GeneratedName(sourceName)
        select new CodeGenerationDiagnostic(
            true,
            propertyName,
            [definedName.Name, sourceName]);
    ...
    static IEnumerable<CodeGenerationDiagnostic> SheetDefinedNameTableDiagnostics(
        ...
        where table.Worksheet.Name == sheet.Name
            && propertyName == options.GeneratedName(table.Name)
        select new CodeGenerationDiagnostic(
            true,
            propertyName,
            [definedName.Name, table.Name]);
