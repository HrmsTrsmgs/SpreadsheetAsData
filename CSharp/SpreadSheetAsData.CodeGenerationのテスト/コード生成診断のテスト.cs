...
public sealed class コード生成診断のテスト
{
    ...
    [Fact]
    public void テーブルの生成プロパティ名が生成BookのReplaceメソッド名と衝突した場合に診断します()
    {
        GeneratedCodeInspection
            .GenerateDiagnostics(
                BasicStructureExcelFilePath,
                options => options.NameMappings = new() { ["sales_detail"] = "Replace" })
            .Should().ContainEquivalentOf(
                new CodeGenerationDiagnostic(
                    true,
                    "Replace",
    ...
    [Fact]
    public void テーブルの生成プロパティ名が生成BookのValidateStructureメソッド名と衝突した場合に診断します()
    {
        GeneratedCodeInspection
            .GenerateDiagnostics(
                BasicStructureExcelFilePath,
                options => options.NameMappings = new() { ["sales_detail"] = "ValidateStructure" })
            .Should().ContainEquivalentOf(
                new CodeGenerationDiagnostic(
                    true,
                    "ValidateStructure",
    ...
    [Fact]
    public void テーブルの生成プロパティ名が生成Bookの型名と衝突した場合に診断します()
    {
        GeneratedCodeInspection
            .GenerateDiagnostics(
                BasicStructureExcelFilePath,
                options => options.NameMappings = new() { ["sales_detail"] = "BasicStructureBook" })
            .Should().ContainEquivalentOf(
                new CodeGenerationDiagnostic(
                    true,
                    "BasicStructureBook",
    ...
    [Fact]
    public void 同じBook型の定義名とシートの生成プロパティ名が衝突した場合に診断します()
    {
        GeneratedCodeInspection
            .GenerateDiagnostics(
                @"TestData\コード生成\衝突なし\定義名.xlsx",
                options => options.NameMappings = new() { ["book.main_cell"] = "SalesData" })
            .Should().ContainEquivalentOf(
                new CodeGenerationDiagnostic(
                    true,
                    "SalesData",
