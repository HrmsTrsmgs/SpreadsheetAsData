...
型付き読み取りコードを生成すると、ブック、シート、テーブル、行データを表す型を使用して、文字列指定を減らした読み取りもできます。
...
現行のコード生成は、読み取り用C#ソース文字列を生成するAPIとして実装しています。
生成コードのパッケージ提供方法や、生成元ファイルパスの扱いは、初回公開準備の中で整理します。
...
現在は、当初から構想していた定義名、Excelテーブル、列名による読み取り、オブジェクトマッピング、型付き読み取りコード生成の初期実装を進め、公開前の説明、サンプル、パッケージ構成を整える段階です。
