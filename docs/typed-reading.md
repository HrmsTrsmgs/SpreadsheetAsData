...
コード生成では、ブック名、シート名、テーブル名、列名などのExcel由来名を、まずC#識別子として使う本文へ変換します。
Book型、Sheet型、Table型、行データ型、プロパティ名は、その識別子本文をそれぞれの生成箇所へ適用します。

初期実装では、まず次の対応を確認します。

* camelCaseのExcel由来名をPascalCaseの識別子本文へ変換する
* ブック名からBook型名を生成する
* シート名からSheet型名を生成する
* テーブル名からTable型名と行データ型名を生成する
* 列名から行データ型のプロパティ名を生成する
...
テストでは、識別子本文の変換規則と、その本文をBook、Sheet、Table、行データ型、プロパティへ適用する規則を分けて確認します。
すべての生成箇所で、すべての入力パターンを重複して確認することは避けます。
